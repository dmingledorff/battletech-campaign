  ...
  `;
    ...
    function showMissionModal(m) {
      ...
      document.getElementById('mapModalBody').innerHTML = `
        ...
        <div id="missionUnitList">
            <p class="text-muted small mb-0">Loading units...</p>
        </div>
    `;
      ...
      // Fetch units after modal opens
      fetch(`/missions/getUnitsForMap/${m.mission_id}`)
        .then(r => r.json())
        .then(units => {
          const container = document.getElementById('missionUnitList');
          if (!units.length) {
            container.innerHTML = '<p class="text-muted small mb-0">No units assigned.</p>';
            return;
          }
          container.innerHTML = `
                <p class="text-muted small fw-bold mb-1">Units (${units.length})</p>
                <ul class="list-unstyled mb-0 small">
                    ${units.map(u => `
                        <li class="py-1 border-bottom border-secondary">
                            <a class="link-info" href="/units/${u.unit_id}">
                                ${u.unit_chain ?? u.name}
                            </a>
                            <span class="text-muted ms-1">(${u.unit_type})</span>
                        </li>
                    `).join('')}
                </ul>
            `;
